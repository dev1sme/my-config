...
collect_config() {
    ...
    ui_note "ssh-agent là gì?" \
        "Chạy ngầm, giữ private key đã unlock trong RAM." \
    ...
    if ui_confirm "Thêm key vào ssh-agent sau khi tạo?" y; then
        ADD_TO_AGENT=true
    else
        ADD_TO_AGENT=false
    fi
...
EOF
    ...
    ui_note "Thêm Host cho GitHub / server (nano ~/.ssh/config)" \
        "Host github.com" \
        "    HostName github.com" \
        "    User git" \
        "    IdentityFile $KEY_FILE" \
...
print_pubkey() {
    ...
    if command -v xclip &>/dev/null; then
        echo "$pubkey" | xclip -selection clipboard && info "Đã copy vào clipboard (xclip)."
    elif command -v xsel &>/dev/null; then
        echo "$pubkey" | xsel --clipboard --input && info "Đã copy vào clipboard (xsel)."
