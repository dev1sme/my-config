...
collect_config() {
    echo ""
    echo -e "${BLUE}============================================================${NC}"
    echo -e "${BLUE}  Cấu hình SSH Key (macOS)${NC}"
    echo -e "${BLUE}============================================================${NC}"
    echo ""
    ...
    echo ""
    echo -e "${BLUE}============================================================${NC}"
    echo -e "${BLUE}  Thêm key vào ssh-agent + macOS Keychain?${NC}"
    echo -e "${BLUE}============================================================${NC}"
    echo ""
    echo -e "  ${YELLOW}ssh-agent + Keychain${NC} trên macOS:"
    echo    "    macOS tích hợp ssh-agent với Keychain, giữ key persistent"
    echo    "    qua các lần reboot — không cần ssh-add lại sau mỗi lần mở máy."
    echo ""
    echo -e "  ${GREEN}[Y] Thêm vào agent + Keychain${NC} ${GREEN}(khuyên dùng)${NC}"
    ...
    echo ""
    if prompt_yn "Thêm key vào ssh-agent + Keychain sau khi tạo?" "y"; then
        ADD_TO_AGENT=true
        info "Sẽ thêm vào ssh-agent + Keychain."
    else
        ADD_TO_AGENT=false
        info "Bỏ qua. Thêm thủ công sau bằng: ssh-add --apple-use-keychain $KEY_FILE"
    fi
    echo ""
...
# ============================================================
# 3. Thêm key vào ssh-agent và macOS Keychain
# ============================================================
add_to_agent() {
    ...
    # Kiểm tra agent có thực sự chạy không
    local agent_status
    ...
    if ssh-add -l 2>/dev/null | grep -qF "$(ssh-keygen -lf "${KEY_FILE}.pub" 2>/dev/null | awk '{print $2}')"; then
        ...
        # --apple-use-keychain: lưu passphrase vào macOS Keychain (persistent qua reboot)
...
EOF
    ...
    echo -e "    ${GREEN}open -e${NC} ~/.ssh/config"
